...
namespace WLHnChart
{
	public class HnChart101 : UserStrategyBase
	{
		int extendedBars = 0;
		int lastBarIdx = 0;
		string SP500 = "0SP500";     //"^GSPC"; // ^GSPC, .SPX, SPY, SP500
		string NASDAQ = "0NASDAQ";   //"^IXIC"; // ^GSPC, .SPX, SPY, SP500
		bool isIndex = false;

		//string tradeFile = @"Z:\trades.csv";
		string tradeFile = @"C:\WLD\2018-LeeMP\trades.csv";
		string mpFile = @"C:\WLD\2018-LeeMP\mp.csv";
		string wooFile = @"C:\WLD\2018-LeeMP\woo.csv";

		//create indicators and other objects here, this is executed prior to the main trading loop
		public override void Initialize(BarHistory bars)
		{
			// Have some space in the right
			bars.ExtendedBars = this.extendedBars;
			...
			this.lastBarIdx = bars.Count - 1 - extendedBars;
		...
		protected void dailyChart(BarHistory bars)
		{
			...
			showTrades(bars, tradeFile);
			showMP(bars, mpFile);
			showWoo(bars, wooFile);
			showVolume(bars);
		...
		{
			StartIndex = 200;
			BarHistory SPX = GetHistory(bars, SP500);
		...
		{
			Color upColor = System.Drawing.ColorTranslator.FromHtml("#1D13E3");
			Color downColor = System.Drawing.ColorTranslator.FromHtml("#D30E9F");
			...
			for (int bar = 0; bar < bars.Count; bar++)
			{
				...
				{
					SetSeriesBarColor(myVolume, bar, upColor);
				}
				else
				{
					SetSeriesBarColor(myVolume, bar, downColor);
				}
			}
        ...
        {	
			...
			foreach (KeyValuePair<int, string> entry in markers)
			{
				...
				if (woo == "WOO")
				{
					DrawBarAnnotation("\n" + "▲", entry.Key, false, Color.FromArgb(160, 0, 255, 0), 14, true);
				}
				else
				{
					DrawBarAnnotation("\n" + "▲", entry.Key, false, Color.FromArgb(160, 255, 0, 0), 14, true);
				}
			}
			DrawHeaderText("▲ - WOO");
		}
		...
		protected void showTrades(BarHistory bars, string tradeFile)
		{
			...
			if (bars.Symbol == SP500 || bars.Symbol == NASDAQ)
			{
				...
				if (symbol == bars.Symbol)
				{
					...
					if (sellDateStr != "")
					{
						...
						if (sellBar > 0)
						{
							...
							DrawLine(sellBar - 1, sellPrice, sellBar + 1, sellPrice, Color.Brown, 3);
						}
					}
				}
			}
